...
Job descriptions tend to get written backwards. Someone opens the last version for a similar role, swaps a couple of lines, changes the title, and posts it. Nobody sits down and asks what this specific job actually needs from a person. The result is a list of demands aimed at the company's convenience: twenty bullet points, a decade of required experience for a job a sharp two-year candidate could do, an unnecessary degree, and jargon only an insider would parse. The strong, qualified candidate reads it, counts that they meet eight of the twenty bullets, decides they are not what you are looking for, and quietly closes the tab. The posting did its job too well. It filtered out exactly the people you were hoping to reach.
...
```json
{
  "stem": "A hiring manager keeps getting too few applicants for an open role and asks why AI is well suited to rewriting the job description. What is the most accurate reason from this lesson?",
  "options": [
    {"id":"a","label":"Rewriting a job description is mostly a clarity, framing, and tone problem, and the HR pro supplies the real facts while the model handles the phrasing.","is_correct":true,"explanation":"Correct. The model does not need to invent anything. You supply the real role, the genuine must-haves, and what makes the team worth joining, and it turns that into a posting written for the reader."},
    {"id":"b","label":"AI knows the current job market better than any recruiter, so it can decide on its own which requirements to keep.","is_correct":false,"explanation":"The model does not know what your role truly requires or what is true about your team. You decide the requirements; the model only helps phrase them."},
    {"id":"c","label":"AI can pull salary and benefits data from your company systems automatically to fill in the posting.","is_correct":false,"explanation":"This is not what the lesson describes, and it is not why AI fits this task. The strength is reframing the substance you provide, not retrieving internal facts on its own."}
  ]
}
```
...
```json
{
  "template": "When a model drafts a job description, watch for inflated or {{1}} requirements it imports from generic templates, because those quietly shrink and skew your {{2}} pool.",
  "blanks": [
    {"id":"1","accept":["exclusionary","excluding","unnecessary"],"ideal":"exclusionary"},
...
```json
{
  "question": "You need a posting for a mid-level operations analyst, and you keep getting too few applicants. Which prompt produces a description that attracts strong candidates instead of filtering them out?",
  "before_prompt": "Write a job description for an operations analyst.",
  "after_prompt": "Draft a job description for a mid-level operations analyst. Day to day, the role pulls weekly performance data, builds simple dashboards, and flags process bottlenecks to the ops team. The genuine must-haves are comfort with spreadsheets, clear written communication, and one to two years in an analytical role. Everything else, including SQL, a specific tool, and a degree, is a nice-to-have, not a requirement. Write it in plain language to attract a capable candidate who may not tick every box, keep the requirements list short and honest, avoid jargon or inflated experience asks, and lead with what makes the work interesting. Then flag any line that could wrongly exclude a qualified person.",
  "changes": [
    "Describes what the role actually does, so the posting reflects the real job instead of a generic template.",
    "Separates true must-haves from nice-to-haves, which stops the requirement list from filtering out strong applicants.",
    "Names the audience and goal and asks the model to flag exclusionary lines, so jargon and inflated asks do not quietly shrink the pool."
  ]
}
```
...
```json
{
  "stem": "The AI-drafted posting lists 'bachelor's degree required' and '7+ years experience' for a role you know a strong two-year candidate could do, even though you never asked for those. What should you conclude?",
  "options": [
    {"id":"a","label":"It is fine, because higher requirements always produce higher-quality applicants.","is_correct":false,"explanation":"Higher bars do not mean better hires. They mean fewer and narrower applicants. An inflated requirement screens out capable people the job never needed to exclude."},
    {"id":"b","label":"The model imported inflated requirements from generic templates, so you should cut them down to the genuine must-haves before posting.","is_correct":true,"explanation":"Correct. Those lines came from the template average baked into the model, not from you. Trimming them to real must-haves widens the pool and stops strong candidates from self-selecting out."},
    {"id":"c","label":"The posting should list even more requirements to be thorough and look serious.","is_correct":false,"explanation":"More requirements make the problem worse. The goal is an honest, short list of genuine must-haves, not an exhaustive wishlist that deters the right candidates."}
  ]
}
```
...
Take a real job description you use, ideally one for a role you struggle to fill, and rewrite it to the standard in this lesson. Before you open the AI, decide what the job actually requires so the model has true material to work with and you have a clear standard to judge the draft against.

- Mark every requirement in your current JD as a genuine must-have or a nice-to-have, and be honest about which inflated lines crept in from old templates, then write a prompt that gives the model the real day-to-day, your honest must-have list, and what makes the team worth joining.
- Ask for a plain-language posting aimed at a capable candidate who may not tick every box, leading with what makes the work interesting rather than the list of demands.
- Review the draft for any requirement that would wrongly exclude someone and any coded or exclusionary language, fix it, then compare it to your original and note how many strong applicants the old version was quietly turning away.
