...
Clients live in the dark, and it scares them. They do not know what discovery means, why a hearing got pushed, or whether silence from your office is good news or bad. Clear, timely communication is one of the most valued things a legal professional offers, and it is also one of the most repetitive. Status updates, document requests, appointment confirmations, explanations of next steps, delay notices. You write the same kinds of messages over and over, with only the names and the facts changing. That repetition is exactly where AI helps, if you use it the right way.
...
```json
{
  "stem": "What is the core idea behind using AI for routine client communication?",
  "options": [
    {
      "id": "a",
      "label": "Template the recurring message with bracketed placeholders, and fill in the confidential specifics and legal substance yourself.",
      "is_correct": true,
      "explanation": "Correct. The shape of a status update or document request repeats across matters, so you build the reusable shell with AI and keep the confidential details and your judgment off the tool."
    },
    {
      "id": "b",
      "label": "Hand each individual client matter to the AI so it can write the most accurate, customized message.",
      "is_correct": false,
      "explanation": "That means pasting confidential matter details into the tool, which is the exact thing to avoid. You template the recurring message, not the individual case."
    },
    {
      "id": "c",
      "label": "Let the AI both draft the wording and decide what to tell the client about their case.",
      "is_correct": false,
      "explanation": "AI can shape wording, but what a client is told about their matter is legal substance and professional judgment, which stays with you."
    }
  ]
}
```
...
```json
{
  "stem": "A colleague pastes a client's real name and a settlement figure into a public AI chat, then deletes the conversation. What actually happened?",
  "options": [
    {
      "id": "a",
      "label": "The data was already sent to an outside service when it was pasted, so deleting the chat does not undo the disclosure.",
      "is_correct": true,
      "explanation": "Correct. Deletion only clears your screen. The confidential details left your control the moment they were submitted, so the breach already occurred."
    },
    {
      "id": "b",
      "label": "Deleting the chat removes the data from the AI provider, so confidentiality is restored.",
      "is_correct": false,
      "explanation": "Deletion does not pull the data back from the outside server. The disclosure happened on submission and cannot be reversed by clearing the chat."
    },
    {
      "id": "c",
      "label": "No problem occurred, because only the name and a number were shared, not the full case file.",
      "is_correct": false,
      "explanation": "A name tied to the existence of a matter is confidential on its own, and a settlement figure is sensitive negotiation data. Both leaving your control is a breach."
    },
    {
      "id": "d",
...
```json
{
  "question": "Same client status update, two prompts. Notice which one keeps client confidentiality intact.",
  ...
  "after_prompt": "Create a reusable, plain-language template for a client status update letter. It should reassure the client, summarize where things stand in general terms, explain the next step, and set realistic expectations for timing. Mark every fill-in part with brackets like [client name], [matter description], and [next step], and use no real names, parties, or dollar figures.",
  "changes": [
    "Builds a reusable template instead of pasting a real client's name, opposing party, and settlement figure into a public tool, which would breach confidentiality.",
    "Uses bracketed placeholders like [client name] and [matter description] so the confidential specifics get filled in offline or in an approved private tool, never typed into the AI.",
    "Keeps the legal substance, whether to counter and at what number, out of the prompt entirely, since that is the professional's judgment and not a templating task."
  ]
}
```
...
```json
{
  "stem": "You need a delay notice for a real client matter. Which prompt applies the lesson correctly?",
  "options": [
    {
      "id": "a",
      "label": "Paste the client's name and the reason for the delay so the AI can write the most personal, accurate notice.",
      "is_correct": false,
      "explanation": "This puts confidential details into the tool. The identity and the matter facts should stay in brackets and be filled in offline."
    },
    {
      "id": "b",
      "label": "Ask for a reusable delay-notice template with brackets like [client name] and [new date], honest and calm, that does not promise any particular result, then fill in the specifics yourself.",
      "is_correct": true,
      "explanation": "Correct. You template the recurring message with placeholders, keep the tone reassuring without over-promising, and add the confidential details by hand."
    },
    {
      "id": "c",
      "label": "Ask the AI to write a notice that reassures the client the delay means the case is going well.",
      "is_correct": false,
      "explanation": "That over-promises an outcome you cannot guarantee. Reassure the process, that delays are normal, not the result of the case."
    }
  ]
}
```
...
Build a starter client communication library so your most common messages are ready to reuse. Pick the three messages you send clients most often, for example a status update, a document request, and a delay or schedule-change notice. For each one, have AI draft a plain-language, reassuring template with every fill-in part marked in brackets and no real client details anywhere. Then make each template sound like your firm, and confirm it sets honest expectations without promising a result.

- Ask AI to create each template using only generic placeholders like [client name], [matter description], and [next step], with no real names, parties, or figures.
- Review and edit each draft so the tone matches how your firm speaks to clients, and remove any line that over-promises an outcome.
- Save the three finished templates where you can reuse them, and fill in confidential specifics offline or in an approved private tool, never in a public one.
